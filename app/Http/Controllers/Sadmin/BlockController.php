...
Class BlockController extends Controller
{
	...
	public function __construct(){
		...
		$this->SADMIN_ROUTE_NAME = CustomHelper::getSadminRouteName();
	...
	public function get_blocks(Request $request){
		...
		->addColumn('action', function(Blocks $data) {
			...
			return '<a title="Edit" href="' . route($routeName.'.blockes.edit',$data->id.'?back_url='.$BackUrl) . '"><i class="fa fa-edit">Edit</i></a>&nbsp;&nbsp;&nbsp;
			';
		})
...
public function add(Request $request){
    ...
    if(is_numeric($id) && $id > 0){
        ...
        if(empty($blockes)){
            return redirect($this->SADMIN_ROUTE_NAME.'/blockes');
        }
    }
    ...
    if($request->method() == 'POST' || $request->method() == 'post'){
        ...
        if(empty($back_url)){
            $back_url = $this->SADMIN_ROUTE_NAME.'/blockes';
        }
    ...
    return view('sadmin.blocks.form', $data);
